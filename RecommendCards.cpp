...
int RecommendCards::HelpPla(struct fDdz * pDdz, bool IsSim)
{
	...
	if (IfPassTwice(pDdz))
	{
		...
		{
			if (pDdz->iOnHand[0] == pDdz->iOnHand[1] == pDdz->iOnHand[2])//三条，直接出
			{
				iTmp[0] = pDdz->iOnHand[0];
				iTmp[1] = pDdz->iOnHand[1];
	...
	else//跟牌
	{
		...
		{
			if (IsSim||(playerType==2&&pDdz->iDownPlayerCards<=3)|| (playerType == 3 && pDdz->iUpPlayerCards <= 3))
				Help3SingleSim(pDdz);
			else
				Help3Single(pDdz);
		}
...
int RecommendCards::Help3Single(struct fDdz * pDdz)
{
	...
	if (iLastMaxPoint <= -1 && iSingleCount <= iThreeCount)return 0;

	iCards = pSplitCards->startSplitCard(pDdz->iOnHand, 7);
	if ((playerType == 2 && pDdz->iDownPlayerCards <=5)|| (playerType == 3 && pDdz->iUpPlayerCards == 2))//如果下家或者上家地主报单或者报双，出最大的单牌
	{
		int i;
		for (i = 0; iCards[i] != -1; i++);//1 -2 4 -2 5 -2 -1  0 1 2 3 4 5 6
		int iTmp[3]= { -1,-2,-1 };
		if (i >= 2)
		{
			if ((iCards[i - 2] == 52 || iCards[i - 2] == 53 )&& iLastMaxPoint < 12)i = i - 2;
			iTmp[0] = iCards[i - 2];
			if (GetLegalCards(pDdz, iTmp))
			{
				return iPlaCount - iCount;
...
int RecommendCards::Help6ThreeDouble(struct fDdz * pDdz)
{
	...
	{
		iCards = pSplitCards->startSplitCard(iCopyOnHand, 7);
		if (iCards[0] > -1 && iCards[0] < 24)//如果存在9以下的单牌，优先出三带一
			return 0;
	}
